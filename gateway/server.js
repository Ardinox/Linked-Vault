...
async function callC(
  ...
  try {
    ...
    console.log(
      `[GATEWAY] Final Payload sending to C:`,
...
const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, SECRET_KEY, (err, user) => {
    if (err) return res.status(401).json({ error: "Invalid Token or session expired" });
...
app.use((err, req, res, next) => {
  ...
  if (err.type === "entity.too.large") {
    return res
      .status(413)
      .json({ error: "File too large. Max limit is 50MB." });
  }
